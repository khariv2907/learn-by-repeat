<table class="table table-bordered decks-table">
    <thead class="thead-dark">
    <tr>
        <th scope="col" class="table-col-id">#</th>
        <th scope="col">Title</th>
        <th scope="col" class="table-col-repeat">Last Repeat</th>
        <th scope="col" class="table-col-actions-200">Actions</th>
    </tr>
    </thead>
    <tr class="table-info">
        <td colspan="4">Essential Grammar in Use</td>
    </tr>
    <tr>
        <td class="text-center">1</td>
        <td>Unit 8</td>
        <td class="text-center">29.07.2020</td>
        <td class="text-center">
            <button type="button" class="btn btn-outline-success btn-sm">Repeat Today</button>

            <div class="btn-group" role="group">
                <button id="btnGroupDrop1" type="button" class="btn btn-outline-info btn-sm dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    More
                </button>
                <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                    <a class="dropdown-item" href="#">View History</a>

                    <div class="dropdown-divider"></div>

                    <a class="dropdown-item" href="#">Repeat Next Day</a>
                    <a class="dropdown-item" href="#">Repeat After 3 Days</a>
                    <a class="dropdown-item" href="#">Repeat In a Week</a>

                    <div class="dropdown-divider"></div>

                    <a class="dropdown-item" href="#">Disable</a>
                </div>
            </div>

        </td>
    </tr>
</table>
